...
| SM-7 | Extraction rate-limit resilience (OpenAI TPM throttle + model lever) | **VERIFIED 2026-07-25** (spec: `extraction-rate-limit-resilience.md`; renumbered SM-6→SM-7 to resolve a parallel-work label collision with the mention-attr fix; 4 units, 8 ACs, +37 tests, full suite 2097 pass, all 4 verify gates PASS; ready to merge → real build+deploy → smoke gpt-4o flip) | Post-SM-1 smoke root-caused: acquisition healthy (`pdf_ok=2`) but both full-text papers got OpenAI `429`'d → 0 entities. 5 concurrent extractors each send the full paper (~50–100k tok) → bursts past `gpt-4-turbo`'s 30k TPM; existing blind-backoff retry can't fix a structurally over-budget workload. Spec: **shared process-global TPM token-bucket** (reuse `TokenBucketRateLimiter`; `OPENAI_TPM` env, default 30000; rate=TPM/60) that every extract call reserves against *before* firing (heuristic estimate `len(text)/4 + max_tokens`); **honor `Retry-After`** on the reactive 429 path (parse-miss logs WARN — no silent revert to blind backoff); **`OPENAI_EXTRACTION_MODEL` env** (default unchanged `gpt-4-turbo`) as the real throughput lever; **flip CI smoke to `gpt-4o`** so `Ingest + Assert` proves entities>0 (prod default untouched); up-front INFO when predicted wait >5s (legible stall, not a hang). Throttle = correctness safety-net; model/tier flip = throughput fix (arithmetic: throttle alone never makes gpt-4-turbo viable for full papers). No input reduction (out of scope), no `tiktoken`. **Tests do NO real sleeping** (assert `wait_estimate` math + captured sleep durations). 8 ACs. Next: `/constellize:feature:implement`. |
