...
| SEG-3 | [Run-in and letter-spaced abstracts](https://github.com/djjay0131/agentic-kg/blob/master/llm/features/seg3-run-in-abstracts.md) | **IMPLEMENTED 2026-09-18** | Four abstract conventions match none of `^abstract\s*$`. **Shipped and measured on the committed corpus: abstracts 1/8 → 7/8, +9,347 chars, `cskg2` byte-identical.** Recall-comparison-valid papers 1/8 → 6/8 — **CONCAT-valid, which is not the same as correctly segmented; see the SEG-7 row.** Six mutations (loose delimiter, universal de-spacing, two-letter `_LETTER_SPACED`, discarded run-in remainder, and both length-guard reverts) verified red. The no-label `cskg2` case remains with SEG-4. |
| SEG-4 | [Journal vocabulary + under-segmentation detector](https://github.com/djjay0131/agentic-kg/blob/master/llm/features/seg4-journal-vocabulary-under-segmentation.md) | **IMPLEMENTED 2026-09-18** | Nature *Scientific Data* vocabulary, positional abstract, and a 40%-of-body detector. **Shipped, all three parts.** Measured on the committed corpus: `cskg2` 23,317 → **35,435** chars, 1 → 4 sections, gold entity groups **23 → 25 of 25 reachable** (of 30 curated) — recovering `scientific knowledge graph`, the chain's spine concept. The positional abstract finds **1,210 chars, matching the hand-verified gold boundary**, taking the set to **8/8 abstracts on the committed corpus** and closing cause (3) there. *Two caveats that must travel with those numbers:* the fixture has its title page cut away, so 1,210 is what **any** prefix-before-first-heading rule scores — the rule's real risk surface is absent from this corpus and is covered by synthetic guards plus a reconstruction of the recorded PDF line widths, **not** by a PDF measurement; and re-running the PDF arm after PR-3 is an **open obligation** before any production-pipeline claim. Eight mutations verified red. Two deviations recorded: the detector gained `_MIN_BODY_SECTIONS_FOR_CHECK = 3` (at N ≤ 2 the 40% threshold is below 1/N and cannot fail to fire), and the spec's "five warnings, zero false positives" is **not reproducible on the committed corpus** — 8 warnings across 7 papers, roughly half of them artifacts of the keep-list-filtered denominator. |
...
|---|---------|--------|-------|
| SEG-7 | Four-section keep-list discards content even when boundaries are perfect | **Needs Decision** (then spec) — **now also gated on section-TYPE correctness, see below** | Highest priority — the only cause that reaches the gold labels. `_build_extractor_section_text` (`ingestion.py:198`) keeps exactly `abstract`/`introduction`/`methods`/`experiments`. Measured body coverage ranges 3.9%–71.7%; `kg_construction_survey` loses 18 of 22 chain entities, `llm_ontology_gen` 8 of 12 (including `scientific knowledge graph`, the chain's spine concept). Sharpest case is `fact_completion`: Section V (`Use Case: AI-KG`, 6,502 chars) is excluded as `use_case` and is the densest source of chain-recurring entities — `support score` occurs **zero** times in the extractor input. Three fix options in increasing order of change: (1) add `use_case`/`application`; (2) also add `results`/`background` (recovers `empire` at 35.2% and `llm_ontology_gen`); (3) invert the filter — keep everything except references/acknowledgments/appendices/author bios. Option 3 **requires SEG-6 first** (the survey would send 181k chars to one prompt). **Decide before diffing**: if the keep-list changes, the affected `paper_<slug>.txt` fixtures must be regenerated and their reviews redone, not patched. **NEW PREREQUISITE (2026-09-18, from the PR #69 review):** today all four kept types are concatenated into one blob, so confusion *between* keep-list types is free and invisible to character recall. Measured on the committed corpus: **7/8 papers are CONCAT-valid but only 4/8 are correctly section-TYPED**. `cskg` — one of two reconciled-gold papers — produces **no `methods` section at all**; its 14,988-char gold methods span is absorbed into a 20,271-char `introduction` and still scores 99.9% recall. `fact_completion` also loses `methods`, `kg_validation_hitl` loses `experiments`; all three are SEG-5 misses. **Inverting the keep-list is exactly the change that makes those type errors start costing**, so SEG-7 needs the TYPED column at 8/8, not the CONCAT column. `scripts/measure_segmentation.py --corpus committed` reports both. |
| SEG-6 | [No upper length guard between segmenter and the LLM call](https://github.com/djjay0131/agentic-kg/blob/master/llm/features/seg6-extractor-input-length-guard.md) | **IMPLEMENTED 2026-09-18** | **Shipped.** `MAX_EXTRACTOR_CHARS = 120_000` in `_build_extractor_section_text`, cutting at a paragraph boundary then a word boundary then a hard slice, with a WARNING naming the paper and the drop. Measured: `kg_construction_survey` sends **244,742** chars against a **7,152**-char gold (34.2x) and is 4.9x the next largest paper; the cap is 2.4x the largest healthy input (49,511) so it cannot fire on a correctly segmented paper — asserted against `seg_baseline.json` and the frozen fixtures, both of which are unchanged. The corrected figure is 244,742, not 244,748. Still a **prerequisite for SEG-7 option 3**, and it does NOT fix the cause (SEG-5 / SEG-11). |
| SEG-5 | Methods/experiments sections are usually not named "Methods" or "Experiments" | **Needs Decision** (then spec) — **(b) done via SEG-4; (d) MEASURED AND REJECTED 2026-09-18** | The stubborn one: **7 of 12 wanted sections unmatched, only 2 of 6 methods sections found**, and **5 of the 7 misses survive a perfect SEG-1 fix**, for four distinct reasons. (a) *Named after the contribution* — `The Computer Science Knowledge Graph`, `III. SciCheck`; no keyword list can catch these. (b) *Journal vocabulary* — `Technical Validation`; fixable via SEG-4. (c) *Descriptive sentence headings* — `4. Integrating LLMs and HiL into the SCICERO validation`, containing no method keyword at all. (d) *Pattern too strict* — `5. Experiment design and implementation` fails because `experiment(?:s\|al)?\s*(?:setup\|settings)?\s*$` allows only `setup`/`settings` as suffix; `IV. RESEARCH APPROACH` fails because the approach pattern admits only `our`/`proposed` as prefix. ~~**(d) is cheap**~~ — **measured and rejected 2026-09-18.** Relaxing the anchors so a heading *containing* a keyword matches instead of requiring near-exact equality mints **166 new headings across the eight committed fixtures, of which 2 are wanted — 2:164** (`IV. RESEARCH APPROACH` and `5. Experiment design and implementation`; the rest are body sentences containing *method* / *model* / *approach* / *evaluation*). An earlier PDF-corpus run put it at 2:108 — different corpus, same numerator, same conclusion. Worse than the `[A-Z]` variant SEG-1 rejected for costing 13,137 chars, and every spurious heading also *splits* the real section that contained the line. Viable **only as a rider on SEG-11**, which is what would suppress the false positives. Not built. Also note (b) `Technical Validation` is now fixed by SEG-4. **Counter figures, re-measured:** the hand-labelled heading table in `scripts/segment_ground_truth.py` now scores **38/48 = 79%**, up from **27/48 = 56%** at `7108c7d`. (a) and (c) cannot be fixed by pattern-matching; options are a **positional fallback** (span between last front-matter section and first results/evaluation section → methods; works for `cskg`, `fact_completion`, `kg_validation_hitl`), **don't filter at all** (see SEG-7 option 3), or LLM-assisted heading classification (likely more machinery than warranted). Note the current failure is **silent** — an unmatched methods heading quietly removes the paper's core technical content from every extractor's input. |
...
| SEG-12 | Semantic (LLM/embedding) heading classification instead of enumerated names | **Needs Decision** — after SEG-7 | **Raised 2026-09-08.** The pattern table enumerates section names, and publishers/authors name sections arbitrarily, so the vocabulary can never be complete. Proposal: match *similar* heading names semantically rather than by enumeration. **Measured against the hand-labelled headings in `scripts/segment_ground_truth.py`** (which is a ready-made eval set): current patterns got **27/48 = 56%**. *(Denominator corrected 2026-09-18: that table holds **48** boundaries, not 42; the numerator 27 was right. Post-SEG-3 and SEG-4 it is **38/48 = 79%**, measured, which is close to the 86% this entry predicted for the pair.)* The 15 misses are 6 abstract conventions (SEG-3) + 1 no-label (SEG-4) + 2 journal vocabulary (SEG-4) + 3 over-strict anchors (SEG-5d) + 1 descriptive sentence + **2 named after the contribution** (`III. SciCheck`, `The Computer Science Knowledge Graph`). SEG-3+SEG-4 alone take this to **86%**, relaxed anchors to ~93%. So the *measured* marginal value of name-similarity here is 1–4 headings — and the 2 hardest are proper nouns that **no name-based method can classify**, LLM or embedding. The real unlock is classifying with **context** (position, sibling headings, first paragraph), which patterns structurally cannot do. **Why this waits for SEG-7:** the only reason inclusion precision matters is the four-section keep-list. Exclusion vocabulary (`references`/`acknowledgments`/`appendix`) is already **8/8 = 100%** accurate with today's patterns vs 64% for inclusion, so inverting the filter (SEG-7 option 3) largely dissolves this problem with no new machinery. SEG-4 also found the perverse incentive: mapping `Data Records`→`methods` *gains* 6,138 chars purely by mislabelling, because the keep-list rewards it. **Hard prerequisites if built:** (1) SEG-11 first — unmatched candidate lines run 31–278/paper, ~90% author names and table cells, so LLM-classifying them would mint phantom sections *with model confidence attached*; (2) determinism — `Paper.taxonomy_hash` + the AC-21 skip-check assume idempotent re-ingestion, so classifications must be cached/persisted (E-7 persists `normalization_audit` on the Paper node as precedent). **Shape if approved:** patterns first, LLM fallback for the residue only, following `cross_entity_normalizer` (closed-set Pydantic `Literal`, self-validation gates, confidence threshold, audit trail); `L-1` is the cost lever. Cost is not the objection — one call per paper covering its whole heading list is trivial beside the 5–6 extraction calls already made. **Counter-argument that stands regardless:** at D-1 scale (thousands of papers, arbitrary publishers) enumeration does not generalise, and 8 papers understates the problem. Likely the right destination, not the current bottleneck. |
...
| SM-10 | [DOI/identifier-targeted ingestion](https://github.com/djjay0131/agentic-kg/blob/master/llm/features/doi-targeted-ingestion.md) | **IMPLEMENTED** (PR #50) | `agentic-kg ingest` was query-only; couldn't hit an exact curated set by DOI. Added `--dois`/`--dois-file` (mutually exclusive with `--query`) resolving via the S2/OpenAlex by-DOI path through the normal ingest loop; `CITES` among the set populates; unresolvable DOIs reported (`unresolved:<doi>`). Unblocked the CS-KG ground-truth run (8/8 resolved vs 2/8 by title search). Surfaced 2026-07-27 during the first staging validation. |
...
Segmenter defects — gates the ground-truth diff, and through it D-2 / V-1
    ...
    Agreed working order (2026-09-08): SEG-5 ──► SEG-11 ──► SEG-7 ──► SEG-12
      SEG-5  recovers the contribution-named / over-strict misses
      SEG-11 removes the phantom headings that make any classifier unsafe
      SEG-7  inverting the keep-list is the intended direction; it also
             decides whether SEG-12 is worth building at all
      SEG-12 semantic heading classification, revisited after SEG-7
