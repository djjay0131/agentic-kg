# Active Context — Archive (through 2026-07)

Historical entries moved out of `activeContext.md` by a Constellize
`memory:revise` on 2026-09-17, when the live file had reached 679 lines with
two duplicate `## Current Work Focus` sections and a stray `# docs-stats`
block whose figures were four months stale.

**Read this for provenance only.** Everything here describes the state of the
project at the time it was written; several entries were already superseded
when they were archived. The live file is the authority on current state.
...
**✅ Layout migrated (2026-08-31):** governance delta and ADRs moved from `docs/` to `llm/governance/` per agentic-governance ADR-0001 (control plane vs data plane); recorded here as ADR-0002. All moves via `git mv`, history intact. Dead `adr/`/`governance-delta.md` excludes removed from `docs/_config.yml`. Sprint history moved `construction/sprints/` → `llm/sprints/` (v0.5 adds the Sprints slot); `construction/` removed. Version pin v0.2 → v0.5.
...
**Deploy Master recovery — the FULL gate chain (each merge revealed the next latent gate, since the workflow had literally never run):**
1. **PR #27** (merged `4bdfb5b`): WIF + staging env + ingest-Job + worker→job. Necessary for auth, NOT the startup cause. Run still `startup_failure`.
2. **PR #28** (merged): `update-manifest` referenced undefined `needs.build` (actionlint-caught). Run still `startup_failure`.
3. **PR #29** (merged `ba18a9f`): **THE startup root cause** — repo default GITHUB_TOKEN=`read`, the `build` job calls reusable `build-images.yml` (requests `id-token: write`), a reusable call can't request more than the caller grants → GitHub rejects at startup. Granted `build` job `id-token: write` (+ same fix on deploy-branch). **Run finally STARTED (in_progress, no startup_failure) — AC-5 achieved.** Then `test` job FAILED.
4. **PR #30** (merged `814f4e4`): `test` job's `ruff check packages/core/src packages/core/tests` hit 172 PRE-EXISTING test-tree violations (SM-5). Project standard (code-review.yml) lints src-only; narrowed the deploy gate to match. Run 29224559187 in_progress at last check.
